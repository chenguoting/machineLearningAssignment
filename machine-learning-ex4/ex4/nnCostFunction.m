...
z2 = [ones(size(a1, 1), 1) a1] * Theta1'; % m x hidden_layer_size
a2 = sigmoid(z2); % m x hidden_layer_size
z3 = [ones(size(a2, 1), 1) a2] * Theta2'; % m x num_labels
a3 = sigmoid(z3); % m x num_labels
H = a3; % m x num_labels

Y = zeros(size(y, 1), num_labels); % m x num_labels
for i=1:size(y, 1)
    Y(i, y(i)) = 1;
end
...
delta2 = (delta3 * Theta2(:, 2:end)) .* sigmoidGradient(z2); % m x hidden_layer_size
